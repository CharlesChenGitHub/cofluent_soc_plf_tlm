...
#include "riscv-vp/common/dmi.h"
#include "iss.h"
#include "riscv-vp/rv32/mmu.h"
...
struct CombinedMemoryInterface : public sc_core::sc_module,
	...
	tlm_utils::simple_initiator_socket<CombinedMemoryInterface> isock[2];
	tlm_utils::tlm_quantumkeeper &quantum_keeper;
	...
	inline void _do_transaction(tlm::tlm_command cmd, uint64_t addr, uint8_t *data, unsigned num_bytes) {
		...
		isock[0]->b_transport(trans, local_delay);
		isock[1]->b_transport(trans, local_delay);
