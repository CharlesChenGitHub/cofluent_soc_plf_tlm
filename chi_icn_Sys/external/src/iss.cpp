...
uint32_t ISS::get_csr_value(uint32_t addr) {
	...
	auto read_mstatus = [=](csr_mstatus &x, uint32_t mask) { return x.reg & mask; };
	auto read_mip = [=](csr_mip &x, uint32_t mask) { return x.reg & mask; };
	auto read_mie = [=](csr_mie &x, uint32_t mask) { return x.reg & mask; };
	auto read_fcsr = [=](csr_fcsr &x, uint32_t mask) { return x.reg & mask; };
	...
	switch (addr) {
		...
		case MSTATUS_ADDR:
			return read_mstatus(csrs.mstatus, MSTATUS_MASK);
		case SSTATUS_ADDR:
			return read_mstatus(csrs.mstatus, SSTATUS_MASK);
		case USTATUS_ADDR:
			return read_mstatus(csrs.mstatus, USTATUS_MASK);

		case MIP_ADDR:
			return read_mip(csrs.mip, MIP_READ_MASK);
		case SIP_ADDR:
			return read_mip(csrs.mip, SIP_MASK);
		case UIP_ADDR:
			return read_mip(csrs.mip, UIP_MASK);

		case MIE_ADDR:
			return read_mie(csrs.mie, MIE_MASK);
		case SIE_ADDR:
			return read_mie(csrs.mie, SIE_MASK);
		case UIE_ADDR:
			return read_mie(csrs.mie, UIE_MASK);
		...
		case FCSR_ADDR:
			return read_fcsr(csrs.fcsr, FCSR_MASK);
...
template <class csr_type>
void write_csr(csr_type& x, uint32_t mask, uint32_t value) {
...
void ISS::set_csr_value(uint32_t addr, uint32_t value) {
	...
	switch (addr) {
        ...
        case SATP_ADDR: {
			...
			write_csr<csr_satp> (csrs.satp, SATP_MASK, value);
            // std::cout << "[iss] satp=" << boost::format("%x") % csrs.satp.reg << std::endl;
        } break;

		case MTVEC_ADDR:
			//write(csrs.mtvec, MTVEC_MASK);
			write_csr<csr_mtvec> (csrs.mtvec, MTVEC_MASK, value);
			break;
		case STVEC_ADDR:
			//write(csrs.stvec, MTVEC_MASK);
			write_csr<csr_mtvec> (csrs.stvec, MTVEC_MASK, value);
			break;
		case UTVEC_ADDR:
			//write(csrs.utvec, MTVEC_MASK);
			write_csr<csr_mtvec> (csrs.utvec, MTVEC_MASK, value);
			break;

		case MEPC_ADDR:
			//write(csrs.mepc, pc_alignment_mask());
			write_csr<csr_mepc> (csrs.mepc, pc_alignment_mask(), value);
			break;
		case SEPC_ADDR:
			//write(csrs.sepc, pc_alignment_mask());
			write_csr<csr_mepc> (csrs.sepc, pc_alignment_mask(), value);
			break;
		case UEPC_ADDR:
			//write(csrs.uepc, pc_alignment_mask());
			write_csr<csr_mepc> (csrs.uepc, pc_alignment_mask(), value);
			break;

		case MSTATUS_ADDR:
			//write(csrs.mstatus, MSTATUS_MASK);
			write_csr<csr_mstatus> (csrs.mstatus, MSTATUS_MASK, value);
			break;
		case SSTATUS_ADDR:
			//write(csrs.mstatus, SSTATUS_MASK);
			write_csr<csr_mstatus> (csrs.mstatus, SSTATUS_MASK, value);
			break;
		case USTATUS_ADDR:
			//write(csrs.mstatus, USTATUS_MASK);
			write_csr<csr_mstatus> (csrs.mstatus, USTATUS_MASK, value);
			break;

		case MIP_ADDR:
			//write(csrs.mip, MIP_WRITE_MASK);
			write_csr<csr_mip> (csrs.mip, MIP_WRITE_MASK, value);
			break;
		case SIP_ADDR:
			//write(csrs.mip, SIP_MASK);
			write_csr<csr_mip> (csrs.mip, SIP_MASK, value);
			break;
		case UIP_ADDR:
			//write(csrs.mip, UIP_MASK);
			write_csr<csr_mip> (csrs.mip, UIP_MASK, value);
			break;

		case MIE_ADDR:
			//write(csrs.mie, MIE_MASK);
			write_csr<csr_mie> (csrs.mie, MIE_MASK, value);
			break;
		case SIE_ADDR:
			//write(csrs.mie, SIE_MASK);
			write_csr<csr_mie> (csrs.mie, SIE_MASK, value);
			break;
		case UIE_ADDR:
			//write(csrs.mie, UIE_MASK);
			write_csr<csr_mie> (csrs.mie, UIE_MASK, value);
			break;

		case MIDELEG_ADDR:
			//write(csrs.mideleg, MIDELEG_MASK);
			write_csr<csr_32> (csrs.mideleg, MIDELEG_MASK, value);
			break;

		case MEDELEG_ADDR:
			//write(csrs.medeleg, MEDELEG_MASK);
			write_csr<csr_32> (csrs.medeleg, MEDELEG_MASK, value);
			break;

		case SIDELEG_ADDR:
			//write(csrs.sideleg, SIDELEG_MASK);
			write_csr<csr_32> (csrs.sideleg, SIDELEG_MASK, value);
			break;

		case SEDELEG_ADDR:
			//write(csrs.sedeleg, SEDELEG_MASK);
			write_csr<csr_32> (csrs.sedeleg, SEDELEG_MASK, value);
			break;

		case MCOUNTEREN_ADDR:
			//write(csrs.mcounteren, MCOUNTEREN_MASK);
			write_csr<csr_mcounteren> (csrs.mcounteren, MCOUNTEREN_MASK, value);
			break;

		case SCOUNTEREN_ADDR:
			//write(csrs.scounteren, MCOUNTEREN_MASK);
			write_csr<csr_mcounteren> (csrs.scounteren, MCOUNTEREN_MASK, value);
			break;

		case MCOUNTINHIBIT_ADDR:
			//write(csrs.mcountinhibit, MCOUNTINHIBIT_MASK);
			write_csr<csr_mcountinhibit> (csrs.mcountinhibit, MCOUNTINHIBIT_MASK, value);
			break;

		case FCSR_ADDR:
			//write(csrs.fcsr, FCSR_MASK);
			write_csr<csr_fcsr> (csrs.fcsr, FCSR_MASK, value);
			break;
